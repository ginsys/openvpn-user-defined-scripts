...
class ClientConnectScript(ovpnscript.OpenVPNScript):
    ...
    def check_config(self):
        ldapconfigkeys = [
            ...
            'ldap_group_baseDN']
        if any([x in self.config for x not in ldapconfigkeys]):
    ...
    def check_group_member(self, con, userDN, cn, username):
        ...
        if username == cn:
            ...
            sortedgroups = sorted(self.config['groups'].keys())
            ...
            if self.ldap_memberof(
                    con, userDN, self.config['groups'][username]):
                group = username
        return group
    ...
    def validate_user(self, common_name, username):
        ...
        if userDN is None:
            self.exit1(
                'ldap user with uid %s from certificate common name was not found' %
                common_name)
        ...
        if groupname is None:
            self.exit1(
                'no matching group membership for %s/%s %s' %
                (common_name, username, userDN))
    ...
    def get_ip(self, username, profile, session_id):
        ...
        if ip[3] + count > 255:
            self.exit1(
                'sorry, no support yet for subnets crossing /24 boundaries')
        ...
        if ip is None:
            self.exit1(
                'could not find a free ip for %s/%s' %
                (username, profile))
    ...
    def cc(self, action, cc_file=None):
        ...
        self.log('%s request from %s/%s  %s:%s' %
                 (action, common_name, username, client_ip, client_port))
        ...
        if action == 'connect':
            ...
            self.exit0(
                'disconnected ip %s from %s/%s' %
                (ip, common_name, username))
